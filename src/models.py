...
class BaseCoralModel(ABC):
    ...
    def optimize_threshold(self, X_val: pd.DataFrame, y_val: pd.Series) -> float:
        ...
        try:
            ...
            logger.info(
                f"Optimal threshold for {self.model_name}: {best_threshold:.3f} (F1: {best_f1:.4f})"
            )
            return best_threshold

        except Exception as e:
            logger.error(f"Error optimizing threshold for {self.model_name}: {e}")
            return 0.5
    ...
    def evaluate(self, X_test: pd.DataFrame, y_test: pd.Series) -> Dict[str, float]:
        ...
        try:
            ...
            logger.info(
                f"{self.model_name} evaluation complete. Accuracy: {metrics['accuracy']:.4f}"
            )
            return metrics
    ...
    def cross_validate(self, X: pd.DataFrame, y: pd.Series) -> Dict[str, float]:
        ...
        try:
            ...
            logger.info(
                f"Cross-validation complete. Mean accuracy: {cv_scores['cv_accuracy_mean']:.4f} ± {cv_scores['cv_accuracy_std']:.4f}"
            )
            return cv_scores
...
class CoralXGBoostModel(BaseCoralModel):
    ...
    def __init__(self, config: Dict):
        ...
        self.model = xgb.XGBClassifier(
            ...
            max_depth=self.xgb_config.get("max_depth", 8),
            learning_rate=self.xgb_config.get("learning_rate", 0.05),
            subsample=self.xgb_config.get("subsample", 0.85),
            colsample_bytree=self.xgb_config.get("colsample_bytree", 0.85),
            colsample_bylevel=self.xgb_config.get("colsample_bylevel", 0.85),
            reg_alpha=self.xgb_config.get("reg_alpha", 0.1),
            reg_lambda=self.xgb_config.get("reg_lambda", 1.0),
            min_child_weight=self.xgb_config.get("min_child_weight", 3),
            gamma=self.xgb_config.get("gamma", 0.1),
            scale_pos_weight=self.xgb_config.get("scale_pos_weight", 4.5),
    ...
    def fit(self, X_train: pd.DataFrame, y_train: pd.Series) -> "CoralXGBoostModel":
        ...
        try:
            ...
            if len(numeric_cols) > 0:
                X_train_clean[numeric_cols] = X_train_clean[numeric_cols].fillna(
                    X_train_clean[numeric_cols].median()
                )
    ...
    def predict_proba(self, X_test: pd.DataFrame) -> np.ndarray:
        ...
        try:
            ...
            # Fill numeric columns with median
            if len(numeric_cols) > 0:
                X_test_clean[numeric_cols] = X_test_clean[numeric_cols].fillna(
                    X_test_clean[numeric_cols].median()
                )
...
class CoralNeuralNetwork(BaseCoralModel):
    ...
    def _build_model(self, input_dim: int):
        ...
        try:
            ...
            hidden_layers = self.nn_config.get("hidden_layers", [256, 128, 64, 32])
            dropout_rate = self.nn_config.get("dropout_rate", 0.4)
            learning_rate = self.nn_config.get("learning_rate", 0.0005)

            # Build improved model with batch normalization
            model = keras.Sequential()

            # Input layer with batch normalization
            model.add(layers.Dense(hidden_layers[0], input_dim=input_dim))
            ...
            logger.info(
                f"Neural network architecture built: {len(hidden_layers)} hidden layers"
            )
            return model
    ...
    def fit(self, X_train: pd.DataFrame, y_train: pd.Series) -> "CoralNeuralNetwork":
        ...
        try:
            ...
            for col in X_train_numeric.columns:
                if (
                    X_train_numeric[col].dtype == "category"
                    or X_train_numeric[col].dtype == "object"
                ):
            ...
            class_weights = self.nn_config.get("class_weight", {0: 0.55, 1: 2.45})

            # Train the model with class weights
            history = self.model.fit(
                X_train_scaled,
                y_train,
                epochs=self.nn_config.get("epochs", 150),
                batch_size=self.nn_config.get("batch_size", 64),
                validation_split=self.nn_config.get("validation_split", 0.25),
    ...
    def predict_proba(self, X_test: pd.DataFrame) -> np.ndarray:
        ...
        try:
            ...
            for col in X_test_numeric.columns:
                if (
                    X_test_numeric[col].dtype == "category"
                    or X_test_numeric[col].dtype == "object"
                ):
                    # Convert categorical to numeric codes
            ...
            logger.error(
                f"Error predicting probabilities with Neural Network model: {e}"
            )
            raise
    ...
    def _calculate_feature_importance(
        ...
        try:
            logger.info(
                "Calculating feature importance for Neural Network using permutation method"
            )
            ...
            baseline_accuracy = accuracy_score(
                y_train, (baseline_predictions > 0.5).astype(int)
            )
            ...
            for i, feature_name in enumerate(self.feature_names):
                ...
                permuted_accuracy = accuracy_score(
                    y_train, (permuted_predictions > 0.5).astype(int)
                )
            ...
            logger.error(
                f"Error calculating feature importance for Neural Network: {e}"
            )
            # Fallback: use uniform importance
            uniform_importance = 1.0 / len(self.feature_names)
            self.feature_importance_ = {
                name: uniform_importance for name in self.feature_names
            }

    def _custom_cross_validate(
        self, X: pd.DataFrame, y: pd.Series, cv_folds: int, random_state: int
    ) -> Dict[str, float]:
        """
        ...
        try:
            ...
            for fold, (train_idx, val_idx) in enumerate(cv.split(X, y)):
                ...
                self.model.fit(
                    ...
                    y_fold_train,
                    epochs=min(
                        50, self.nn_config.get("epochs", 100)
                ...
                fold_f1 = f1_score(
                    y_fold_val, y_pred, average="weighted", zero_division=0
                )
            ...
            logger.info(
                f"Neural Network cross-validation complete. Mean accuracy: {cv_scores['cv_accuracy_mean']:.4f} ± {cv_scores['cv_accuracy_std']:.4f}"
            )
            return cv_scores
...
def create_model(model_type: str, config: Dict) -> BaseCoralModel:
    ...
    Args:
        model_type: Type of model ('xgboost', 'neural_network', or 'ensemble')
        config: Configuration dictionary
    ...
    if model_type.lower() == "xgboost":
        ...
        return EnsembleCoralModel(config)
    else:
        raise ValueError(
            f"Unknown model type: {model_type}. Supported types: 'xgboost', 'neural_network', 'ensemble'"
        )


def compare_models(
    models: Dict[str, BaseCoralModel], X_test: pd.DataFrame, y_test: pd.Series
) -> pd.DataFrame:
    """
...
class EnsembleCoralModel(BaseCoralModel):
    """
    Ensemble model combining XGBoost and Neural Network predictions.
    """
    ...
    def __init__(self, config: Dict):
        ...
        self.xgb_model = None
        self.nn_model = None
        self.ensemble_weights = config.get("ensemble", {}).get("weights", [0.6, 0.4])

    def fit(self, X_train: pd.DataFrame, y_train: pd.Series) -> "EnsembleCoralModel":
        """Train both base models."""
        logger.info("Training ensemble model components...")

        try:
            # Train XGBoost
            self.xgb_model = CoralXGBoostModel(self.config)
            self.xgb_model.fit(X_train, y_train)

            # Train Neural Network if available
            if TENSORFLOW_AVAILABLE:
                self.nn_model = CoralNeuralNetwork(self.config)
    ...
    def predict_proba(self, X_test: pd.DataFrame) -> np.ndarray:
        ...
        try:
            ...
            if self.nn_model is not None:
                ...
                # Weighted ensemble
                ensemble_proba = (
                    self.ensemble_weights[0] * xgb_pred_proba
                    + self.ensemble_weights[1] * nn_pred_proba
                )
            else:
                # Use only XGBoost if NN not available
                ensemble_proba = xgb_pred_proba

            # Convert to binary classification format
            prob_negative = 1 - ensemble_proba
            prob_positive = ensemble_proba

            return np.column_stack([prob_negative, prob_positive])

        except Exception as e:
            logger.error(f"Error predicting ensemble probabilities: {e}")
            raise
    ...
    def cross_validate(self, X: pd.DataFrame, y: pd.Series) -> Dict[str, float]:
        ...
        logger.info("Performing cross-validation for Ensemble")

        try:
            # Use XGBoost cross-validation as proxy since ensemble uses both models
            if self.xgb_model is not None:
                return self.xgb_model.cross_validate(X, y)
            else:
            ...
            logger.error(f"Error in cross-validation for Ensemble: {e}")
            return {
                "cv_accuracy_mean": 0.0,
                "cv_accuracy_std": 0.0,
                "cv_f1_mean": 0.0,
    ...
    def _calculate_ensemble_feature_importance(self) -> None:
        """Calculate weighted ensemble feature importance."""
        try:
            xgb_importance = self.xgb_model.get_feature_importance()

            if self.nn_model is not None:
                nn_importance = self.nn_model.get_feature_importance()

                # Weighted combination
                ensemble_importance = {}
                for feature in self.feature_names:
                    xgb_imp = xgb_importance.get(feature, 0)
                    nn_imp = nn_importance.get(feature, 0)
                    ensemble_importance[feature] = (
                        self.ensemble_weights[0] * xgb_imp
                        + self.ensemble_weights[1] * nn_imp
                    )
            else:
                ensemble_importance = xgb_importance

            self.feature_importance_ = ensemble_importance
